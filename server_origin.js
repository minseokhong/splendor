...
io.on('connection', (socket) => {
	...
	socket.on('new user', (player) => {
		...
		if (players.length === 4 || gameInProgress) {
			...
			socket.broadcast.emit('new online observer', player.username, socket.id);
			...
			socket.broadcast.emit('new online player', player.username, socket.id);
	...
	socket.on('new game', () => {
		...
		for (let i = 0; i < players.length; i++) {
			io.to(players[i].id).emit('show board', players);
		}
		io.emit('chat message', `A new game has begun!`);
		whoseTurn();
	});

	socket.on('update player status', (status) => {
		if (players.findIndex(player => player.id === socket.id) !== -1) {
			players[players.findIndex(player => player.id === socket.id)].status = status;
		}

		socket.broadcast.emit('show scoreboard', players);
	})
